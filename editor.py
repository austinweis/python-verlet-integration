...
try:
    ...
    sticks = file["sticks"]
    paths = file["forms"]
    pointProperties["static"] = file["properties"]["static"]
    pointProperties["follow_mouse"] = file["properties"]["follow_mouse"]
                    ...
                    if mouseX < width:
                        ...
                        for i in range(len(points)): 
                            if round(mouseX/cellSize) == points[i][0] and round(mouseY/cellSize) == points[i][1]:
                                _static = False
                                _follow = False
                                for p in pointProperties["static"]:
                                    if p == i:
                                        _static = True
                                for p in pointProperties["follow_mouse"]:
                                    if p == i:
                                        _follow = True
                                if _static == False and _follow == False:
                                    pointProperties["static"].append(i)
                                if _static == True and _follow == False:
                                    print(pointProperties["static"])
                                    pointProperties["static"].remove(i)
                                    pointProperties["follow_mouse"].append(i)
                                if _static == False and _follow == True:
                                    pointProperties["follow_mouse"].remove(i)
                                _newPoint = False
                ...
                if stickMode == False:
                    for i in range(len(points)): 
                        if round(mouseX/cellSize) == points[i][0] and round(mouseY/cellSize) == points[i][1]:
                            ...
                            p = len(pointProperties["static"])
                            while p > 0:
                                p-=1
                                if pointProperties["static"][p] == i:  
                                    pointProperties["static"].remove(i)
                                    continue
                                if pointProperties["static"][p] > i: 
                                    pointProperties["static"][p] -= 1
                            p = len(pointProperties["follow_mouse"])
                            while p > 0:
                                p-=1
                                if pointProperties["follow_mouse"][p] == i:  
                                    pointProperties["follow_mouse"].remove(i) 
                                    continue
                                if pointProperties["follow_mouse"][p] > i: 
                                    pointProperties["follow_mouse"][p] -= 1                            
                            break
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            if event.button == pygame.BUTTON_LEFT:
                ...
                if clearButton.collidepoint(mouseX, mouseY):
                    ...
                    sticks = []
                    paths = []
                    pointProperties["static"] = []
                    pointProperties["follow_mouse"] = []
                if simulationButton.collidepoint(mouseX, mouseY):
                    simulationButtonColor = (255, 255, 0)
                    main.main()
                    pygame.quit()     
